...
def get_base_data(params, data=None):
    ...
    if params.get('train_labeled_fraction'):
        ...
        train_split = (1, 0)
    (x_train_unlabeled, y_train_unlabeled, p_train_unlabeled), (x_train_labeled, y_train_labeled, _) = split_data(
...
def load_data(params):
    ...
    if params['dset'] == 'reuters':
        with h5py.File(os.path.join(params['data_path'], '/reutersidf_total.h5'), 'r') as f:
        ...
        x_train, x_test, y_train, y_test = generate_cc(params.get('n'), params.get('noise_sig'),
