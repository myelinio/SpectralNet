...
def save_hdf5(X, y, name):
    import h5py
    with h5py.File(os.path.join(data_path, './{}.h5'.format(name)), 'w') as f:
        f['data'] = X
        f['labels'] = y
...
def make_reuters_data():
    ...
    with open(os.path.join(data_path, 'rcv1-v2.topics.qrels')) as fin:
    ...
    del did
    for dat in dat_list:
        with open(os.path.join(data_path, dat)) as fin:
            for line in fin.readlines():
